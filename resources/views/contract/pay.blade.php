...
<html>
    ...
    <body>
        <div class="container">
            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="text-center">
                        <h3>ใบเรียกเก็บค่าทำสัญญาห้องพัก #{{ $invoice->invoice }}</h3>
                    </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-condensed">
                                    ...
                                    <tbody>
                                        ...
                                        @if($invoice->ref != "")
                                        <tr>
                                            <td>ค่ามัดจำจากการจองห้องพักที่ชำระมาแล้ว</td>
                                            <td class="text-center"></td>
            ...
            <center>
                @if($invoice->status == 0)
                {!! Form::open(['url' => 'manage_contract/payment', 'method' => 'post']) !!}
                <input type="hidden" name="id" value="{{ $user->id }}">
                <input type="hidden" name="invoice" value="{{ $invoice->invoice }}">
                <input type="hidden" name="month" value="{{ date('m') }}">
                <input type="hidden" name="year" value="{{ date('Y') }}">
                <input type="hidden" name="total" value="{{ $pricebooking+$total }}">
                <button class="btn btn-warning">
                    ชำระค่าใบเรียกเก็บค่าทำสัญญาห้องพัก
                </button>
                {!! Form::close() !!}
                @endif
                @if($invoice->status != 0)
                <a href="" class="btn btn-danger" onclick="close_window();return false;">ปิดหน้านี้</a>
                @endif
            </center>
            <br/>
            <style>
